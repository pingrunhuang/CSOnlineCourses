...
Problem: 
    clustering
Solution:
    Double hashing: h(k) = (ord1(k) + i*ord2(k)) mod m (where m is odd, ord2(k) is pow(2,r) so that they are relatively prime 
"""
...
class OpenAddressHasher:
    ...
    def linear_probing(self, key, i):
        ...
        Probe if the current position is occupied or not, if it is, call it recursively until it return None or out of the size
        """
        h = self.division_hashing(key)
        if self.table.get(h) is None:
            return i
        if h == i: # meaning already traversed all the keys in the table
            return -1
